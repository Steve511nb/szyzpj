export function initPortfolioAdditions(root) {
  ...
  const stats=['data-1.png','data-2.png','data-3.png'].map((f,i)=>`<a href="${A+f}" target="_blank" rel="noopener" aria-label="放大查看账号数据${i+1}">${image(f,'我的账号数据 '+(i+1))}</a>`);
  ...
  const early=Array.from({length:5},(_,i)=>{const file=i===4?'early-5-v2.png':'early-'+(i+1)+'.jpg';return `<a href="${A+file}" target="_blank" rel="noopener" aria-label="放大查看早期参演照片${i+1}">${image(file,'早期参演照片 '+(i+1))}</a>`;});
  const radio=['早安綦江','綦江日报','早安綦江'].map((t,i)=>`<figure>${image('radio-'+(i+1)+'.png',t+'栏目作品截图')}<figcaption>${t}</figcaption></figure>`);
  const pages=document.createElement('div');pages.className='additional-pages';pages.innerHTML=`
    ...
    <section class="contact-end" id="contact-end" aria-label="微信联系"><div class="contact-center">${image('contact-qr.jpeg','宋致远的微信联系二维码')}<p>感谢您的观看，可扫描微信二维码联系我哦～</p></div><img class="footer-people" src="${A}footer-people.png" alt="" aria-hidden="true"></section>`;
